...
class Robot:
    body = Cube(robot_x,robot_y,robot_z, (0.8,0.8,0.8,1), robot_angle).set_dimensions(float(0.3),float(0.2),float(0.2))
    wheel_1 = Cylinder(robot_x-0.3/4,robot_y,robot_z+0.2, (0,0.8,0,1), robot_angle).set_dimensions(float(0.1),float(0.1))
    wheel_2 = Cylinder(robot_x-0.3/4,robot_y,robot_z-0.2, (0,0,0.8,1), robot_angle).set_dimensions(float(0.1),float(0.1))
...
renderer.render_with_shader_rot_cam("pers",render_objects, move_robot, keyboard_interrupt)