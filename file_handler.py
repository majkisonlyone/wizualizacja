        ...
        with open(self.file_name,"r") as file:
            for line in file:
                ...
                if splitted[0] == "Sphere":
                    ...
                    angle_x,angle_y,angle_z = splitted[3].split("angle=")[1].split(",")
                    r,q = splitted[4].split("dim=")[1].split(",")
                    objects.append(Sphere(float(x),float(y),float(z), col, [float(angle_x), float(angle_y), float(angle_z)]).set_dimensions(float(r),int(q)))
                ...
                if splitted[0] == "Cube":
                    ...
                    objects.append(Cube(float(x),float(y),float(z), col, [float(angle_x), float(angle_y), float(angle_z)]).set_dimensions(float(a),float(a),float(a)))
                ...
                if splitted[0] == "Cylinder":
                    ...
                    objects.append(Cylinder(float(x),float(y),float(z), col, [float(angle_x), float(angle_y), float(angle_z)]).set_dimensions(float(r),float(h)))
                
                if splitted[0] == "CameraOrientation":
                    dist = float(splitted[1])
                    angle_left_right = float(splitted[2])
                    angle_top_bottom = float(splitted[3])
                    camera_params = CameraOrientation(dist,angle_left_right,angle_top_bottom)
